...
export function TelegramBriefDeliveryPanel({
  ...
  const submit = async (dryRun: boolean) => {
    ...
    try {
      const response = await fetch("/api/connectors/telegram/briefs", {
        method: "POST",
        headers: {
      ...
      if (!response.ok) {
        throw new Error(payload?.error?.message ?? "Failed to deliver Telegram brief.");
      }
      ...
      setError(
        submissionError instanceof Error
          ...
          : "Failed to deliver Telegram brief."
      );
    } finally {
