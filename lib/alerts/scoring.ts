...
type ProjectSignalContext = {
  ...
  tasks: ExecutiveTask[];
  openTasks: ExecutiveTask[];
  overdueTasks: ExecutiveTask[];
  blockedTasks: ExecutiveTask[];
  risks: ExecutiveRisk[];
  openRisks: ExecutiveRisk[];
  criticalOpenRisks: ExecutiveRisk[];
  teamMembers: ExecutiveTeamMember[];
  overloadedTeamMembers: ExecutiveTeamMember[];
  milestone: ExecutiveMilestone | null;
...
export function buildAlertFeed(
  ...
  const projectAlerts = options.projectId
    ...
    : snapshot.projects.flatMap((project) =>
        buildProjectAlerts(snapshot, project.id, { locale, referenceDate })
      );
  ...
  return {
    ...
    summary: {
      ...
      averageConfidence: average(
        prioritized.map((alert) => round(alert.confidence, 2))
      ),
      averageFreshness: average(
        prioritized.map((alert) => round(alert.freshness, 2))
      ),
    },
    alerts: prioritized,
...
export function buildPortfolioAlerts(
  snapshot: ExecutiveSnapshot,
  options: Pick<AlertOptions, "locale" | "referenceDate"> = {}
): PrioritizedAlert[] {
  const referenceDate = toDate(options.referenceDate ?? snapshot.generatedAt);
  const locale = resolveBriefLocale(options.locale);
  const projectAlerts = snapshot.projects.flatMap((project) =>
    buildProjectAlerts(snapshot, project.id, { locale, referenceDate })
  );
  const atRiskProjects = snapshot.projects.filter(
    (project) => project.status === "at-risk" || project.health < 55
  );
  const criticalProjectAlerts = projectAlerts.filter(
    (alert) => alert.severity === "critical"
  );
  const overdueTasks = snapshot.tasks.filter((task) => isOpenTask(task) && isPast(task.dueDate, referenceDate));
  const overloadedMembers = snapshot.teamMembers.filter(
  ...
  const totalBudgetPlanToDate = snapshot.projects.reduce(
    (sum, project) => sum + calculateBudgetPlanToDate(project, referenceDate),
    0
  );
  const totalBudgetVarianceRatio =
    totalBudgetPlanToDate > 0 ? totalBudgetVariance / totalBudgetPlanToDate : 0;
      ...
      scoreAlert(
        {
          ...
          category: "portfolio",
          severity:
            atRiskProjects.length >= Math.max(2, Math.ceil(snapshot.projects.length * 0.4))
              ? "critical"
              : "high",
          confidence: 0.9,
          title:
          ...
          summary:
            locale === "ru"
              ? `${atRiskProjects.length} из ${snapshot.projects.length} ${formatProjectNoun(
                  snapshot.projects.length,
                  locale
                )} требуют эскалации; лидируют ${formatList(projectNames, locale)}.`
              : `${atRiskProjects.length} of ${snapshot.projects.length} projects currently require escalation, led by ${formatList(
                  projectNames,
                  locale
                )}.`,
          whyItMatters:
          ...
          detectedAt:
            latestTimestamp(
              atRiskProjects.map((project) => project.history.at(-1)?.date ?? snapshot.generatedAt)
      ...
      scoreAlert(
        {
          ...
          summary:
            locale === "ru"
              ? `${overdueTasks.length} ${formatTaskNoun(
                  overdueTasks.length,
                  locale,
                  "overdue"
                )} распределены по ${affectedProjects} ${formatProjectNoun(
                  affectedProjects,
                  locale
                )}.`
          ...
          detectedAt:
            latestTimestamp(overdueTasks.map((task) => task.dueDate).filter(isNonEmptyString)) ??
            snapshot.generatedAt,
          metrics: {
      ...
      scoreAlert(
        {
          ...
          summary:
            locale === "ru"
              ? `Портфель идёт с отклонением ${formatSignedPercent(
                  totalBudgetVarianceRatio,
                  locale
                )} к плану на текущую дату.`
              : `Portfolio spend is ${formatSignedPercent(
                  totalBudgetVarianceRatio,
                  locale
...
export function buildProjectAlerts(
  ...
  if (
    ...
    const criticalOverdue = context.overdueTasks.filter((task) =>
      ...
      scoreAlert(
        {
          ...
          category: "schedule",
          severity:
            context.daysToDeadline < 0 || criticalOverdue > 0
              ? "critical"
              : context.overdueTasks.length >= 2 ||
                ...
                : "medium",
          confidence:
            context.overdueTasks.length > 0 || context.daysToDeadline < 0
              ? 0.96
          ...
          detectedAt:
            (context.overdueTasks.length > 0
              ...
              : context.daysToDeadline < 0
                ? project.dates.end
                : latestTimestamp(
                    context.blockedTasks.map((task) => task.dueDate)
                  )) ??
            snapshot.generatedAt,
          metrics: {
      ...
      scoreAlert(
        {
          ...
          summary:
            locale === "ru"
              ? `${project.name} идёт с отклонением ${formatSignedPercent(
                  context.budgetVarianceRatio,
                  locale
                )} к плану на текущую дату (${formatCurrency(
                  context.budgetVariance,
                  project.budget.currency,
                  locale
                )}).`
              : `${project.name} is ${formatSignedPercent(
                  context.budgetVarianceRatio,
                  locale
                )} versus planned-to-date (${formatCurrency(
                  context.budgetVariance,
                  project.budget.currency,
                  locale
                )}).`,
          ...
          detectedAt: latestProjectTimestamp(project) ?? snapshot.generatedAt,
          metrics: {
      ...
      scoreAlert(
        {
          ...
          category: "risk",
          severity:
            context.criticalOpenRisks.length > 0
              ? "critical"
          ...
          summary:
            locale === "ru"
              ? `Активны ${context.openRisks.length} ${formatRiskCount(
                  context.openRisks.length,
                  locale
                )}, из них ${context.criticalOpenRisks.length} высокого уровня.`
          ...
          detectedAt:
            latestTimestamp(context.openRisks.map((risk) => risk.updatedAt)) ?? snapshot.generatedAt,
          metrics: {
  ...
  if (
    context.overloadedTeamMembers.length > 0 ||
      ...
      scoreAlert(
        {
          ...
          category: "resource",
          severity:
            context.overloadedTeamMembers.length >= 2 ||
          ...
          detectedAt:
            latestTimestamp(
              context.tasks.map((task) => task.dueDate).filter(isNonEmptyString)
            ) ?? snapshot.generatedAt,
          metrics: {
  ...
  if (
    context.planFact.evidence.pendingWorkReports >= 2 ||
      ...
      scoreAlert(
        {
          ...
          category: "delivery",
          severity:
            context.planFact.evidence.pendingWorkReports >= 4 ||
            (context.planFact.evidence.daysSinceLastApprovedReport ?? 0) >= 5
              ? "high"
              : "medium",
          ...
          summary:
            locale === "ru"
              ? `${context.planFact.evidence.pendingWorkReports} отчётов ждут review, последняя подтверждённая запись была ${context.planFact.evidence.daysSinceLastApprovedReport ?? "давно"} дн. назад.`
              : `${context.planFact.evidence.pendingWorkReports} work reports are pending review and the last approved report was ${context.planFact.evidence.daysSinceLastApprovedReport ?? "too many"} days ago.`,
          whyItMatters:
            locale === "ru"
          ...
          detectedAt:
            context.planFact.evidence.lastApprovedWorkReportDate ?? snapshot.generatedAt,
          metrics: {
            pendingWorkReports: context.planFact.evidence.pendingWorkReports,
            daysSinceLastApprovedReport:
              context.planFact.evidence.daysSinceLastApprovedReport ?? 0,
          },
        },
        referenceDate
      )
...
export function scoreAlert(
  ...
  const score =
    SEVERITY_SCORES[draft.severity] * 0.55 +
    clamp(draft.confidence, 0, 1) * 100 * 0.3 +
    freshness * 100 * 0.15;

  return {
    ...draft,
    confidence: round(clamp(draft.confidence, 0, 1), 2),
    freshness,
    score: round(score, 1),
  };
}
...
function buildProjectSignalContext(
  snapshot: ExecutiveSnapshot,
  project: ExecutiveProject,
  referenceDate: Date
  ...
  const criticalOpenRisks = openRisks.filter(
    (risk) => risk.severity >= 4 || risk.probability * risk.impact >= 0.5
  );
  const teamMembers = snapshot.teamMembers.filter((member) =>
    member.projectIds.includes(project.id)
  );
  const overloadedTeamMembers = teamMembers.filter(
    (member) => member.capacity > 0 && member.allocated / member.capacity >= 0.9
  );
  const milestone =
    snapshot.milestones
      .filter(
        (candidate) =>
          candidate.projectId === project.id && candidate.status !== "completed"
      )
      .sort(
        (left, right) =>
  ...
  const budgetVarianceRatio =
    budgetPlanToDate > 0 ? budgetVariance / budgetPlanToDate : 0;
...
function calculateBudgetPlanToDate(
  project: ExecutiveProject,
  referenceDate: Date
): number {
...
function calculateRiskExposure(risks: ExecutiveRisk[]): number {
  return risks.reduce(
    (sum, risk) => sum + risk.probability * risk.impact * (risk.severity / 5),
    0
  );
}

function buildScheduleSummary(
  context: ProjectSignalContext,
  locale: BriefLocale
): string {
  if (context.daysToDeadline < 0) {
    return locale === "ru"
      ? `${context.project.name} уже вышел за плановую дату завершения; открыты ${context.overdueTasks.length} ${formatTaskNoun(
  ...
  if (context.overdueTasks.length > 0) {
    return locale === "ru"
      ? `${context.overdueTasks.length} ${formatTaskNoun(
          context.overdueTasks.length,
          locale,
          "overdue"
        )} и ${context.blockedTasks.length} ${formatTaskNoun(
          context.blockedTasks.length,
          locale,
          "blocked"
        )} замедляют исполнение в ${context.project.name}.`
  ...
  return locale === "ru"
    ? `${context.blockedTasks.length} ${formatTaskNoun(
        context.blockedTasks.length,
        locale,
        "blocked"
      )} ограничивают исполнение в ${context.project.name}.`
...
function buildResourceSummary(
  ...
  if (context.overloadedTeamMembers.length > 0) {
    return locale === "ru"
      ? `${context.overloadedTeamMembers.length} исполнителей уже загружены более чем на 90%, при этом открыты ${context.openTasks.length} ${formatTaskNoun(
  ...
  return locale === "ru"
    ? `${context.openTasks.length} ${formatTaskNoun(
        context.openTasks.length,
        locale,
        "open"
      )} конкурируют за команду из ${context.teamMembers.length} человек.`
    : `${context.openTasks.length} open tasks are competing for a team of ${context.teamMembers.length} people.`;
}

function latestProjectTimestamp(project: ExecutiveProject): string | null {
  return project.history.at(-1)?.date ?? project.nextMilestone?.date ?? project.dates.start;
}
...
function compareAlerts(left: PrioritizedAlert, right: PrioritizedAlert): number {
  return right.score - left.score || right.confidence - left.confidence;
}

function isOpenTask(task: ExecutiveTask): boolean {
  return task.status !== "done" && task.status !== "cancelled";
}

function isPast(value: string | null | undefined, referenceDate: Date): boolean {
  if (!value) {
    return false;
  }
...
function diffInDays(from: Date, to: string): number {
...
function formatRiskCount(count: number, locale: BriefLocale): string {
  if (locale === "ru") {
