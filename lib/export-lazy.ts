 ...
 * Dynamically imports heavy libraries (jspdf, xlsx) only when needed
 * Reduces initial bundle size by ~200KB
 */

export async function downloadProjectsCsv(projects: Project[]) {
  const header = [
    "ID",
    "Проект",
    "Статус",
...
export async function downloadTasksCsv(tasks: Task[]) {
  const header = [
    "ID",
    "Проект",
    "Задача",
...
export async function downloadProjectPdf(project: Project, tasks: Task[], risks: Risk[]) {
  // Dynamically import jspdf only when needed (~100KB)
