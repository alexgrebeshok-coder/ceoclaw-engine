...
import { validateProjectImport } from "@/lib/import/project-import-service";
...
export async function POST(request: NextRequest): Promise<NextResponse> {
  try {
    ...
    const result = await validateProjectImport(files);
    return NextResponse.json(result);
  } catch (error) {
    return serverError(error, "Failed to validate import files.");
  }
}
